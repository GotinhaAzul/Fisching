...
def ativar_buff(buff_def, fonte=None):
    ...
    instancia.setdefault("nome", "Buff culinário")
...
def normalizar_buffs_salvos(buffs_salvos):
    normalizados = []
    for buff in buffs_salvos:
        copia = copy.deepcopy(buff)
        duracao = copia.get("duracao_pescas", 0)
        copia.setdefault("usos_restantes", duracao)
        copia.setdefault("nome", copia.get("id", "Buff"))
        copia.setdefault("efeitos", {})
        copia.setdefault("fonte", copia.get("nome"))
